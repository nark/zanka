...
for i in $ARCHS; do
	if [ ! -f "$TEMP_FILE_DIR/make/$i/Makefile" ]; then
		...
		cd "$SRCROOT/libwired"
		CFLAGS="-g -O2 -arch $i" CPPFLAGS="-I$TEMP_FILE_DIR/make/$i -isysroot $SDKROOT" ./configure --build="$BUILD" --host="$HOST" --srcdir="$SRCROOT/libwired" --enable-warnings --enable-pthreads --enable-ssl --with-objdir="$OBJECT_FILE_DIR/$i" --with-rundir="$TEMP_FILE_DIR/run/$i/libwired" || exit 1
		...
		ln -sf "$TEMP_FILE_DIR/run/$i/libwired/include/wired" "$BUILT_PRODUCTS_DIR/"
	fi
